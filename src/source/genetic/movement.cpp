...
bool organisation::genetic::movements::movement::validate(data &source)
{
    ...
    if(directions.size() < _min_movements)//||(directions.size() >= _max_patterns))
    {
        std::cout << "movement::validate(false): directions out of bounds\r\n"; 
        return false; 
    }
    ...
    for(auto &direction: directions)
    {
        ...
        */
        ...
        //vector direction = it;//std::get<1>(it);
